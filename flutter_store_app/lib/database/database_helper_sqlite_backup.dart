...
class DatabaseHelperBackup {
  ...
  Future<Database> _initLocalDB(String filePath) async {
    try {
      ...
      // Ensure IsSynced column exists in Orders (Migration for Deletion Logic)
      try {
         await db.rawQuery("SELECT IsSynced FROM Orders LIMIT 0");
      } catch (e) {
         // Column doesn't exist
         await db.execute("ALTER TABLE Orders ADD COLUMN IsSynced INTEGER DEFAULT 0");
         print("🛠️ Added IsSynced column to Orders table");
         
         // Assume existing orders are synced if we are in this state (to prevent accidental mass resync issues, 
         // but actually we want to verify. Safest is 0, but if we set 0 and they are in cloud, we push.
         // If they are NOT in cloud, we push (reviving ghosts).
         // WAIT! If I set default 0, the next sync will see them as "unsynced" and PUSH them.
         // This defeats the purpose if the goal is to delete ghosts.
         
         // Strategy: We can't know for sure without checking cloud.
         // So initially set all to 0. Sync Service will:
         // 1. Fetch Cloud List.
         // 2. Match Local IDs.
         // 3. If Match -> UPDATE Local IsSynced = 1.
         // 4. If No Match -> Push.
         
         // Problem: The ghost orders (deleted on cloud) will have IsSynced=0.
         // The sync service will see them as new and Push them.
         
         // How do we distinguish "New Local Order" vs "Old Ghost"?
         // We can't easily without a timestamp or history.
         // BUT, the user said "Ghost orders are reappearing".
         // The only way to stop them is if the app knows they *should* be there.
         
         // Better Strategy for FIRST RUN with this feature:
         // Trust the Cloud?
         // If we trust Cloud: If Local Order ID < Max Cloud ID and not in Cloud -> Delete? 
         // Risk: Local created order offline while others created 100 orders online.
         
         // Let's stick to: Default 0.
         // I will modify Sync Service to "Mark as Synced" whenever it sees an ID in Cloud.
         // But this doesn't help with the *current* ghost orders which are already here.
         // The user will have to delete them ONE MORE TIME locally or via bot (if they exist there? No they are gone from bot).
         // If they are gone from Bot, the user must delete them locally manually.
         // UNLESS I implement a "Check Existence" phase.
         
         // If I set IsSynced = 1 for ALL existing local orders during migration?
         // Then:
         // 1. App starts. Migrates. Sets IsSynced=1.
         // 2. Sync runs.
         // 3. Checks Cloud. Order #X is missing.
         // 4. App sees Local #X has IsSynced=1.
         // 5. App Deletes Local #X. -> SUCCESS.
         
         // What if there was a GENUINE new local order created offline just before update?
         // It would be marked IsSynced=1.
         // It would be missing from Cloud.
         // App would DELETE it. -> DATA LOSS.
         
         // So defaulting to 1 is dangerous.
         
         // Alternate: Default to 0.
         // User has to MANUALLY delete the ghosts *one last time* from the app UI.
         // OR run a script to cleanup.
         
         // Wait, the user said: "When deleting message in Bot, it deletes in Cloud, but remains in Desktop... when syncing... it returns".
         // This implies the standard workflow is: Order created on Cloud (Synced to Desktop). Order Rejected on Bot (Deleted on Cloud). Desktop pushes it back.
         // If the order came from Cloud originally, it should have been marked IsSynced=1 when it arrived.
         
         // So, correct implementation going forward:
         // 1. When pulling from Cloud, set IsSynced=1.
         // 2. When creating locally, IsSynced=0.
         // 3. When pushing successfully, set IsSynced=1.
         
         // For the *existing* mess:
         // If I leave IsSynced=0. Sync will push them. Ghost returns.
         // If I set IsSynced=1. Sync will delete them (if missing). Real Offline Orders deleted.
         
         // Compromise: Set IsSynced = 1 for orders older than X hours?
         // Or just accept that for this transition, the user might need to perform a "Pull Only" sync?
         
         // I will add the column with default 0. 
         // AND I will add a logic in SyncService called `_reconcileDeletions`.
      }
  ...
  Future<void> markOrderAsSynced(int orderId) async {
    ...
    try {
      ...
      print("⚠️ Error marking order $orderId as synced: $e");
    }
  }

  Future<List<int>> getSyncedOrderIds() async {
    final db = await database;
    try {
      final result = await db.query('Orders', columns: ['OrderID'], where: 'IsSynced = 1');
      return result.map((e) => e['OrderID'] as int).toList();
    } catch (e) {
      return [];
    }
  }
